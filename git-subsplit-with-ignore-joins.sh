...
subsplit_publish()
{
	...
	do
		...
		do
			...
			git subtree split -q --ignore-joins --prefix="$SUBPATH" --branch="$LOCAL_BRANCH" "origin/${HEAD}" >/dev/null
			RETURNCODE=$?
			...
			then
				...
				echo "${DEBUG} git subtree split -q --ignore-joins --prefix=\"$SUBPATH\" --branch=\"$LOCAL_BRANCH\" \"origin/${HEAD}\" >/dev/null"
			fi
		...
		do
			...
			git subtree split -q --ignore-joins --annotate="${ANNOTATE}" --prefix="$SUBPATH" --branch="$LOCAL_TAG" "$TAG" >/dev/null
			RETURNCODE=$?

			if [ -n "$VERBOSE" ];
			then
				echo "${DEBUG} git subtree split -q --ignore-joins --annotate=\"${ANNOTATE}\" --prefix=\"$SUBPATH\" --branch=\"$LOCAL_TAG\" \"$TAG\" >/dev/null"
			fi
